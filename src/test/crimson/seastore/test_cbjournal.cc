  ...
  bufferlist& bl)
{
  ...
  decode(crc, bliter);
  for (int i = 0; i < header.extents; i++) {
    extent_info_t ex;
    decode(ex, bliter);
...
struct entry_validator_t {
  ...
  void validate(CBJournal &cbj) {
    blk_paddr_t offset = 0;
    for (int i = 0; i < entries; i++) {
      auto [header, buf] = *(cbj.read_record(addr + offset).unsafe_get0());
...
struct cbjournal_test_t : public seastar_test_suite_t
{
  ...
  auto submit_record(record_t&& record) {
    ...
    auto [addr, seq] = cbj->submit_record(
	  std::move(record),
	  handle).unsafe_get0();
    entries.back().addr = (addr.segment * device->get_block_size()) + addr.offset;
