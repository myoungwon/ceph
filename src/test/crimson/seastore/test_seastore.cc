...
TEST_P(seastore_test_t, log_writes)
{
  run_async([this] {
	...
	*sharded_seastore,
    ...
    //auto t = key_for_test_obj.pop_front();
    ...
    auto leaf_size = crimson::os::seastore::logstore_manager::get_log_leaf_node_block_size();
    uint16_t entry_size = crimson::os::seastore::logstore_manager::LogKVLeafNodeLayout::test_get_entry_size(log_key, log_val);
    ...
    kvs = test_obj.get_omaps(*sharded_seastore, std::string());
      //logger().debug(" after {} exp size {} leaf size {} leaf entries {}", *key_for_test_obj.rbegin(), log_count % leaf_entries, leaf_size, leaf_entries);
