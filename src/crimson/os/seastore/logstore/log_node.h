...
//#define LOG_NODE_BLOCK_SIZE 8192
//#define LOG_LEAF_NODE_BLOCK_SIZE (8192*2)
//#define LOG_LEAF_NODE_BLOCK_SIZE (8192)
...
public:
  ...
  void journal_insert(
    ...
    if (recorder) {
    LOG_PREFIX(Cache::LogKVNodeLayout);
    SUBDEBUG(seastore_t, " journal_insert index {} key {}", _iter.index, key);
      recorder->insert(
	key,
	val);
    ...
    LOG_PREFIX(Cache::LogKVNodeLayout);
    SUBDEBUG(seastore_t, " journal_insert index {} key {}", _iter.index, iter->get_key());
  ...
  void journal_update(
    ...
    assert(recorder);
    LOG_PREFIX(Cache::LogKVNodeLayout);
    SUBDEBUG(seastore_t, " journal_update index {} key {}", _iter.index, iter->get_key());
  ...
  void _insert(
    ...
    LOG_PREFIX(Cache::LogKVNodeLayout);
    SUBDEBUG(seastore_t, "enter _insert");
    if (iter != iter_begin()) {
    ...
    SUBDEBUG(seastore_t, "insert  key off {} key len {} val len {}", node_key.key_off, node_key.key_len, node_key.val_len);
    SUBDEBUG(seastore_t, "insert index {} key {} val len {}", iter.index, iter->get_key(), iter->get_val().length());
  }
  ...
  void _update(
    ...
    _insert(iter, key, val);
    LOG_PREFIX(Cache::LogKVNodeLayout);
    SUBDEBUG(seastore_t, "update index {} key {} val len {}", iter.index, iter->get_key(), iter->get_val().length());
  }
...
struct LogNode : LogicalChildNode, LogKVNodeLayout {
  ...
  void logical_on_delta_write() final {
    LOG_PREFIX(Cache::logical_on_delta_write);
    SUBDEBUG(seastore_t, "before this crc{}", calc_crc32c());
    update_delta();
    set_reserved_len(0);
    SUBDEBUG(seastore_t, "after this crc{}", calc_crc32c());
  ...
  get_value_ret get_value(const std::string &key)
  {
    ...
    LOG_PREFIX(Cache::get_value);
    SUBDEBUG(seastore_t, "get_key {} value size {}", iter->get_key(), value.length());
...
public:
  ...
  private:
    ...
    auto get_node_val_ptr() const {
      auto node_key = get_node_key();
    LOG_PREFIX(Cache::get_node_key_ptr);
    SUBDEBUG(seastore_t, "get node ptr {} pos {} key len {} val len {}", (void*)get_node_key_ptr(), pos, node_key.key_len, node_key.val_len);
      return get_node_key_ptr() + sizeof(log_leaf_key_t); //node_key.key_len + node_key.val_len;
    }
    ...
    void set_node_key(log_leaf_key_t _lb) {
      ...
      lb = _lb;
    LOG_PREFIX(Cache::set_node_key);
    SUBDEBUG(seastore_t, "set node start pos {} key pos {} and pos {} key {}", (void*)node->buf, (void*)node->get_node_key_ptr(), pos, (uint16_t)lb.key_len);
    ...
    void set_node_val(const std::string &key, const ceph::bufferlist &val) {
      ...
      assert(val.length() == node_key.val_len);
    LOG_PREFIX(Cache::set_node_key);
    SUBDEBUG(seastore_t, "set node key len {} val len {}", node_key.key_len, node_key.val_len);
      ::memcpy(get_node_val_ptr(), key.data(), key.size());
    SUBDEBUG(seastore_t, "set node val ptr {} str {} to --> ptr {}", (void*)get_node_val_ptr(), (char*)get_node_val_ptr(), (void*)(get_node_val_ptr() + node_key.key_len));
      auto bliter = val.begin();
      bliter.copy(node_key.val_len, get_node_val_ptr() + node_key.key_len);
    }
...
public:
  ...
  void set_last_pos(uint32_t pos) {
    LOG_PREFIX(Cache::set_last_pos);
    SUBDEBUG(seastore_t, "buf {} ", (void*)buf);
  ...
  uint32_t free_space() const {
    LOG_PREFIX(Cache::free_space);
    SUBDEBUG(seastore_t, "capacity {} used_space {}", capacity(), used_space());
  ...
  void _append(const std::string &key, const ceph::bufferlist &val) {
    LOG_PREFIX(Cache::_append);
    SUBDEBUG(seastore_t, "buf {} last_pos {} size {} key len {} val len {}", (void*)buf, get_last_pos(), get_size(), key.size(), val.length());
    iterator prev_iter(this, get_last_pos());
    auto last = prev_iter->get_node_key();
    ...
    if (get_size() >= 1) {
    SUBDEBUG(seastore_t, "last key len {} last val len {}", last.key_len, last.val_len);
      set_last_pos(get_last_pos() + get_entry_size(last.key_len, last.val_len));
    }
    SUBDEBUG(seastore_t, "after last_pos {} ", (uint32_t)(*get_last_pos_ptr()));
    set_size(get_size() + 1);
    SUBDEBUG(seastore_t, "after size {} ", get_size());
  }
  ...
  void journal_append(
    ...
    LOG_PREFIX(Cache::journal_append);
    SUBDEBUG(seastore_t, "buf {} ", (void*)buf);
    recorder->insert(key, val);
    reserved_len += this->get_entry_size(key.size(), val.length());
  }
  ...
  bool is_overflow(size_t ksize, size_t vsize) const {
    LOG_PREFIX(Cache::is_overflow);
    SUBDEBUG(seastore_t, "free space {} key len {} val len {}", free_space(), ksize, vsize);
    return free_space() < get_entry_size(ksize, vsize);
  }

  bool is_overflow(const LogKVLeafNodeLayout &rhs) const {
    LOG_PREFIX(Cache::is_overflow);
    SUBDEBUG(seastore_t, "free space {} ", free_space());
...
struct LogLeafNode 
  ...
  CachedExtentRef duplicate_for_write(Transaction&) final {
    ...
    SUBDEBUG(seastore_t, "duplicate for write ");
    assert(delta_buffer.empty());
  ...
  get_value_ret get_value(const std::string &key)
  {
    ...
    while(iter != iter_end()) {
    LOG_PREFIX(Cache::get_value);
    SUBDEBUG(seastore_t, "leaf get_key {} value size {}", iter->get_key(), iter->get_val().length());
      if (iter->get_key() == key) {
	auto bl = iter->get_val();
	return get_value_ret(
	  interruptible::ready_future_marker{},
  ...
  const_iterator string_lower_bound(std::string_view str) const {
    ...
    while(iter != iter_end()) {
    LOG_PREFIX(Cache::string_lower_bound);
  SUBDEBUG(seastore_t, "str {} get_key{} iter end pos {}",  str, iter->get_key(), iter_end().get_pos());
      if (iter->get_key() < str) {
	target = iter;
      }
      iter++;
  ...
  void list(const std::optional<std::string> &first,
    ...
    std::map<std::string, bufferlist> &kvs)
  {
    LOG_PREFIX(Cache::list);
    auto s = first ? string_upper_bound(*first) : iter_cbegin();
    auto e = last ? string_lower_bound(*last) : iter_cend();
  SUBDEBUG(seastore_t, "leaf s {} e {} pos {} size {} last pos {}", s->get_key(), e->get_key(), e->get_pos(), get_size(), get_last_pos());
    while (s != e) { 
      auto bl = s->get_val();
      kvs[s->get_key()] = bl;
  SUBDEBUG(seastore_t, "sub leaf  s {} e {}",  s->get_key(), e->get_key());
      s++;
    };
  }
