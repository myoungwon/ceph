  ...
  Transaction &t, std::map<std::string, ceph::bufferlist>&& kvs) 
{
  ...
  LOG_PREFIX(LogStoreManager::log_set_keys);
  DEBUGT("enter key {} value size {}", t, p.first, p.second.length() );
	...
	if (can_handle_by_lognode(p.first)) {
	  ...
	  DEBUGT("before omw insert kv ret {} ------- mut {} key {} ", t, *ext, *mut, p.first);
	  mut->insert_kv(p.first, p.second);
	  DEBUGT("after omw insert kv {} ---- mut {} ", t, *ext, *mut);
  ...
  LOG_PREFIX(LogStoreManager::log_set_keys);
  DEBUGT("enter list size {}", t, list.size());
  ...
  const std::string &key, const ceph::bufferlist &value)
{
  auto add_func = [this, &t, &log_root](
	...
	(r->get_head_leaf_laddr() == L_ADDR_NULL && r->tail_laddr != L_ADDR_NULL)) {
      ...
      DEBUGT("before append kv {} ", t, *r);
      mut->append_kv(t, key, value);
      DEBUGT("after append kv {} ", t, *r);
      return log_set_key_iertr::now();
    }
...
#if 0
    if (r->tail) {
      return add_func(r, r->tail, key, value);
    } else if (r->tail_laddr != L_ADDR_NULL) {
#endif
...
{
  LOG_PREFIX(LogStoreManager::remove_kv);
  DEBUGT("first={}, last={}, dst={}", t, first, last, dst);
  if (dst == L_ADDR_NULL) {
  ...
  return log_load_extent<LogLeafNode>(
    ...
    return seastar::do_with(std::move(extent), [&, this](auto &extent) {
      ...
      extent->list(f, l, kvs);
  LOG_PREFIX(LogStoreManager::remove_kv);
  DEBUGT("rbegin={}, last key={} ", t, kvs.rbegin()->first, extent->get_last_key());
      if (!kvs.empty() && kvs.rbegin()->first == extent->get_last_key()) {
      //if (f <= extent->get_last_key() && l >= extent->get_last_key()) {
  LOG_PREFIX(LogStoreManager::remove_kv);
  DEBUGT("first={}, last={}, lastkey={}", t, first, last, extent->get_last_key());
