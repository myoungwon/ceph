...
uint32_t VectorNodeListLayout::entry_array_base() const
{
  ...
  return entry_bytes <= uint32_t(stored->free_begin)
    ? uint32_t(stored->free_begin) - static_cast<uint32_t>(entry_bytes)
...
int VectorNodeListLayout::validate() const
{
  ...
  // exactly with `count` equal-stride rows: entry order defines vector-row
  ...
  const ceph::os::vector_record_t &record)
{
  ...
  // itself, since an entry is a few bytes and a row can be kilobytes.
  const uint32_t old_entries_base = entry_array_base();
  const uint32_t new_entries_base = old_entries_base + aligned_vector;
  std::memmove(
    mutable_data + new_entries_base,
    mutable_data + old_entries_base,
    uint64_t(count) * sizeof(vector_list_entry_le_t));

  char *vector_dest = mutable_data + old_entries_base;
  std::memset(vector_dest, 0, aligned_vector);
  auto vector_it = record.vector_data.cbegin();
  vector_it.copy(row_length, vector_dest);

  vector_list_entry_le_t entry;
  std::memcpy(entry.entry_id, record.entry_id.data(), 8);
  std::memcpy(
    mutable_data + new_entries_base + uint64_t(count) *
      sizeof(vector_list_entry_le_t),
    &entry, sizeof(entry));
  ...
  stored->item_count = count + 1;
  stored->logical_entry_count = count + 1;
  stored->free_begin = new_entries_base +
