...
public:
  ...
  alloc_paddr_iertr::future<> delayed_transaction_paddr_alloc(
    Transaction& t) {
    ...
    return seastar::do_with(
      ...
      [this, &alloc_list, &t](auto& alloc_map, auto& inline_list) mutable {
      for (auto& extent : alloc_list) {
        // extents may be invalidated
        if (extent->state == CachedExtent::extent_state_t::INVALID) {
          continue;
        }
