...
// live in OMAP (see ceph::os::make_vector_omap() in common/vector_record.h)
// under the same entry_id, keyed as _ENTRY_<entry_id>.*. A query joins a
// scanned row back to that OMAP record by entry_id; the LIST page itself
// never stores or interprets those fields.
...
// boundary (entry_array_base()): appending a row shifts the (tiny,
...
// moved-bytes step of its own. header().free_begin marks the end of the
// entry_id array, i.e. the start of free space. There is no metadata
// region: bucket_name, index_name, user_key, placement info, and
// application metadata are not LIST data at all -- see the header comment
// on vector_list_entry_le_t.
//
// Every row in one LIST chain shares its group's (dimension, data_type),
// so every row also has the same aligned byte length. Callers that
// already know that length (derived once from (dimension, data_type), not
// stored per row) pass it into vector_offset_at()/record_at()/
// scan_records()/vector_matrix_view() so this class can index rows in
// O(1) instead of carrying or re-deriving a per-row length.
//
// Invariant: entry order is append order, nothing else -- there is no
...
// bytes moved. A page that cannot fit the next record signals "full";
// the caller allocates a new LIST page and links it via next_page_laddr
// rather than rebuilding or splitting this one.
...
public:
  ...
  // Start of the vector plane: a fixed offset immediately after the
  // header, independent of item_count.
  ...
  // Visits every record in entry (append) order in O(item_count) total.
  // `row_length` as above.
