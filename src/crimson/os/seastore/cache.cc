...
CachedExtentRef Cache::duplicate_for_write(
  ...
  if (i->is_exist_clean()) {
    i->version++;
    assert(i->get_version() < std::numeric_limits<extent_version_t>::max());
  ...
  const journal_seq_t &journal_dirty_tail)
{
  ...
  if (trans_src == Transaction::src_t::TRIM_DIRTY) {
    // in overwrite case, the version was not increased 
  ...
  sea_time_point modify_time)
{
  ...
  if (delta.type == extent_types_t::ROOT) {
	...
	// in partial-overwrite, the version of the initial delta can be non-zero,
	// so we need to call query_cache unconditionally
