  ...
  const std::optional<string> &start)
{
  ...
  return repeat_with_onode<ret_bare_t>(
    ...
    return seastar::do_with(
      omap_list_bare_ret(),
      [this, &t, &onode, start](auto &list) -> omap_list_ret {

      vector<omap_root_le_t> oroots;
	...
	return omap_list(
	  ...
	  OMapManager::omap_list_config_t()
	    .with_inclusive(false, false)
	    .without_max()
	).si_then([this, &list] (auto &&ret) mutable {
  ...
  std::map<std::string, ceph::bufferlist>&& kvs)
{
  return seastar::do_with(
    ...
    [&, keys=std::move(kvs)](auto &omap_manager, auto &root) {
      auto leaf_size = OMAP_LEAF_BLOCK_SIZE;
      if (root.get_type() == omap_type_t::OMAP_SMALL_LEAF) {
	leaf_size = OMAP_LEAF_BLOCK_SIZE_SMALL;
      }
  ...
  std::map<std::string, ceph::bufferlist> &&aset)
{
  ...
  return seastar::do_with(
    ...
    sets[omap_type_t::OMAP_LARGE_LEAF] = std::move(aset);
    std::erase_if(sets[omap_type_t::OMAP_LARGE_LEAF], [&sets](auto &item) {
    ...
    return trans_intr::do_for_each(sets,
      ...
      return _omap_set_kvs(
	onode,
	set.first != omap_type_t::OMAP_SMALL_LEAF ?
	onode->get_layout().omap_root_large_leaf : 
	onode->get_layout().omap_root_small_leaf,
	*ctx.transaction,
	std::move(set.second)
  ...
  omap_keys_t &&keys)
{
  ...
  return seastar::do_with(
    std::move(keys),
    [&ctx, &onode, this](auto &keys) -> tm_ret {
    vector<omap_root_t> oroots;
    oroots.push_back(onode->get_layout().omap_root_large_leaf.get(
      onode->get_metadata_hint(device->get_block_size())));
    oroots.push_back(onode->get_layout().omap_root_small_leaf.get(
      onode->get_metadata_hint(device->get_block_size())));
  ...
  std::string last)
{
  ...
  return seastar::do_with(
    std::move(first),
    std::move(last),
    [&ctx, &onode, this](auto &first, auto &last) -> tm_ret {
    vector<omap_root_t> oroots;
    oroots.push_back(onode->get_layout().omap_root_large_leaf.get(
      onode->get_metadata_hint(device->get_block_size())));
    oroots.push_back(onode->get_layout().omap_root_small_leaf.get(
