...
#include "crimson/osd/seastore/journal.h"
...
public:
   ...
   *
   */
  write_ret append_record(ceph::bufferlist bl, laddr_t addr);
  /*
   * device_write_bl
   *
   ...
   * 	      total length written to CBJournal (bytes)>
   */
  write_ret device_write_bl(paddr_t offset, ceph::bufferlist &bl);

  using read_record_ertr = read_ertr;
  using read_record_ret = read_entry_ertr::future<
   ...
   *
   */
  read_entry_ret read_record(blk_paddr_t offset);
  /*
   * read_super
   *
  ...
  struct cbj_header_t {
    ...
    uint64_t max_size;   // max length of journal
  ...
  read_entry_ret return_record(const bufferlist &bl);
  bool validate_metadata(const bufferlist &bl);
...
WRITE_CLASS_DENC_BOUNDED(crimson::os::poseidonstore::CBJournal::header_t)
