...
SegmentJournal::write_record_ret Journal::write_record(
  record_size_t rsize,
  record_t &&record,
...
SegmentJournal::prep_replay_segments(
  std::vector<std::pair<device_segment_t, segment_header_t>> segments)
{
  logger().debug(
...
SegmentJournal::replay_ret SegmentJournal::replay(
  std::vector<std::pair<device_segment_t, segment_header_t>>&& segment_headers,
