...
// Compiled-in default LIST block size; the actual size is configured via
// seastore_vector_node_list_block_bytes (must be 16384 or 32768).
static constexpr extent_len_t VECTOR_NODE_LIST_DEFAULT_BYTES = 16 << 10;
// Fixed INDEX page size: SeaStore's standard minimum extent unit
// (laddr_t::UNIT_SIZE). Independent of, and never derived from, the
// configured LIST block size -- see vector_node_index_entry_capacity().
static constexpr extent_len_t VECTOR_NODE_INDEX_BYTES = 4 << 10;
...
public:
  ...
  // without ever reading a LIST block.
