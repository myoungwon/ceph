...
#endif
    OP_PUT_VECTOR = 45, // vector OMAP attrset
...
public:
  ...
  /// Put vector keys on oid omap.
  void put_vector(
    const coll_t& cid,
    const ghobject_t &oid,
    const std::map<std::string, ceph::buffer::list> &attrset
    ) {
    using ceph::encode;
    Op* _op = _get_next_op();
  ...
  /// Put pre-encoded vector keys on oid omap.
  void put_vector(
    const coll_t& cid,
    const ghobject_t &oid,
    const ceph::buffer::list& attrset_bl
    ) {
    Op* _op = _get_next_op();
    _op->op = OP_PUT_VECTOR;
    _op->cid = _get_coll_id(cid);
